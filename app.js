...
function setMasterGate(locked) {
  const ids=["surveyTypeCard","existingSearchCard","consumerCard","entryCard","newSurveyCard","myCollectionCard","dashboardCard","correctionCard","adminCard"];
  ids.forEach(id => {
    const el=$(id);
...
function closeAllSurveyForms() {
  ...
  $("surveyTypeCard").classList.remove("hidden");
...
function showExistingSurvey() {
  $("surveyTypeCard").classList.add("hidden");
  $("newSurveyCard").classList.add("hidden");
...
function showNewSurvey() {
  ...
  $("consumerCard").classList.add("hidden");
  $("entryCard").classList.add("hidden");
  $("newSurveyCard").classList.remove("hidden");
  $("newSaveStatus").textContent="";
...
async function openCorrection() {
  ...
  $("correctionCard").classList.toggle("hidden");
  if (!$("correctionCard").classList.contains("hidden")) {
...
async function openDashboard() {
  ...
  $("dashboardCard").classList.toggle("hidden");
  if (!$("dashboardCard").classList.contains("hidden")) {
...
async function openAdmin() {
  ...
  if (!$("adminCard").classList.contains("hidden")) await loadUsers();
}
...
async function v151ExportLocalData() {
  ...
  a.href=url; a.download="survey_local_data_"+new Date().toISOString().slice(0,10)+".csv";
