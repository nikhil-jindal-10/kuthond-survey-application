    ...
    if (state.sessionToken && state.userId) {
      ...
      checkAuth().then(auth => {
        if (!auth.success) {
          ...
          localStorage.removeItem("consumerMobileIsAdmin");
          showLogin();
          setStatus("loginStatus", auth.message || "Please login again.", "error");
          return;
        }
        ...
        // Keep the locally authenticated session when the server is temporarily
        // unreachable; offline survey work remains available.
      });
    } else {
      showLogin();
    }
